...
set thisPath=%~dp0
echo ^<button id = "thisPathID" onclick=""^>!thisPath!^</button^> ^<br^> >>index.html 

call set /a nSubDir = 0
for /f  %%a in ('dir /b /s /ad') do (
	call set /a nSubDir = !nSubDir! + 1
	echo ^<a id = "SubDir!nSubDir!" href="" onclick="fn1(this)"^>%%a^</a^> 	>>index.html
)
...
echo ^<button onclick="me.playId(1)"^>playId(1)^</button^>	>>index.html

echo ^<script^>								>> index.html

echo  function fn1(o)							>>index.html
echo  { var thisPath = document.getElementById("thisPathID");		>>index.html
echo    var s2 = o.innerHTML; var s1 = thisPath.innerHTML;		>>index.html
echo    var s = s2.slice(s1.length,s2.length); s+="/index.html";			>>index.html
echo    o.href=s;//							>>index.html
echo  }									>>index.html
...
echo var me = new mp3Engine("v1.0.6");					>>index.html 
echo //*/	>>index.html
echo ^</script^>							>>index.html
